...
format_ax(
    title=f'''NMI: {nmi_all:.2f}, ARI: {ari_all:.2f} 
...
n_clones_recovered = df['GBC'].unique().size
perc_assigned = df.shape[0] / n_top_all
format_ax(
    title=f'''NMI: {nmi_top:.2f}, ARI: {ari_top:.2f} 
        Clones >=10 cells: {n_clones_recovered}/{n_clones_gt} clones recovered, {perc_assigned*100:.2f}% cells''', 
    ax=axs[1], rotx=90
)
fig.tight_layout()
